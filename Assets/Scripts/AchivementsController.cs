...
public class AchivementsController : MonoBehaviour
{
    ...
    private int allBosses = 1;
    private int allClosedLocations = 2;
    private int allMagicTypes = 2;
    ...
    void Check20Deaths()
    {
        if (SaveSystem.GetInt("countOfDeaths") > 20 || Upd_PlayerControl.Instance.countOfDeaths > 20)
        {
    ...
    {
        if ((SaveSystem.GetInt("countOfDeathsTotal") == 0 && SaveSystem.GetBool("gameCompleted")) || 
           (Upd_PlayerControl.Instance.countOfDeathsTotal == 0 && Upd_PlayerControl.Instance.gameCompleted))
        {
    ...
    void CheckNotes()
    {
        if (SaveSystem.GetInt("countOfNotes") == allNotes || Upd_PlayerControl.Instance.countOfNotes == allNotes)
        {
    ...
    {
        if (SaveSystem.GetInt("countOfKilledBosses") == allBosses || Upd_PlayerControl.Instance.countOfKilledBosses == allBosses)
        {
    ...
    void CheckLocations()
    {
        if (SaveSystem.GetInt("countOfVisitedLoc") >= allClosedLocations || 
            Upd_PlayerControl.Instance.countOfVisitedLoc >= allClosedLocations)
        {
    ...
    void CheckBossHeals()
    {
        if ((SaveSystem.GetInt("countOfUsedHeals") == 0 && SaveSystem.GetBool("gameCompleted")) 
            || (Upd_PlayerControl.Instance.countOfUsedHeals == 0 && Upd_PlayerControl.Instance.gameCompleted))
        {
    ...
    void CheckMagic()
    {
        if (SaveSystem.GetInt("countOfMagic") == allMagicTypes || Upd_PlayerControl.Instance.countOfMagic == allMagicTypes)
        {
    ...
    void CheckChests()
    {
        if (SaveSystem.GetInt("countOfChests") == allSecretChests || Upd_PlayerControl.Instance.countOfChests == allSecretChests)
        {
